...
@section('content_header')
    <h2 class="text-center text-blue" >Add new teacher</h2>
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('teachers.store') }}">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <select class="form-control js-example-basic-multiple" name="sports[]" multiple="multiple" required>
                            @foreach($sports as $sport)
                            <option value="{{ $sport->id}}" {{in_array($sport->id, old('sports',[])) ? 'selected' : '' }}>{{$sport->name}}</option>
                            @endforeach
                        </select>
                        @error('sport')
                            <span class="text-danger">
